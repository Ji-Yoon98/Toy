...
function Bullet() {
    ...
    this.checkHit = function () {
        for (let i = 0; i < enemyArr.length; i++) {
            if (this.y <= enemyArr[i].y && this.x >= enemyArr[i].x && this.x <= enemyArr[i].x + 50) {
                score++;
                this.alive = false;
                this.fire = true;
                enemyArr.splice(i, 1);
...
function render() {
    ...
    for (let i = 0; i < bulletArr.length; i++) {
        if (bulletArr[i].alive) {
            ctx.drawImage(bulletImage, bulletArr[i].x, (bulletArr[i].y -= 7), 32, 32);
        }
        if (bulletArr[i].fire) {
...
function main() {
    if (!gameOver) {
        ...
        ctx.drawImage(gameOverImage, 250, 100, 380, 300);
    }
}
